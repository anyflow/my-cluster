...
- `oauth2-proxy`는 런타임과 선언형 매니페스트에서 제거되었고, Authelia는 IdP 역할만 유지한다.
...
- 런타임 구성에서 제거되었다.
- `apps/oauth2-proxy/*` 선언형 매니페스트는 삭제되었다.
- `Makefile`에는 기존 release/resource cleanup용 `oauth2-proxy-d`만 남긴다.
...
- `oauth2-proxy`는 현재 클러스터/Makefile 기준 제거되었다.
  - 설치 target은 제거되었고 cleanup target `oauth2-proxy-d`만 남긴다.
  - `apps/oauth2-proxy/*` 매니페스트도 삭제되었다.
